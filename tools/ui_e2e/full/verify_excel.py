"""트랙 #83 — 엑셀 검증 + Summary 통계."""
from __future__ import annotations
import io
import sys
from pathlib import Path

sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding="utf-8", errors="replace")

from openpyxl import load_workbook

XLSX = Path(__file__).resolve().parents[3] / "docs" / "TEST_CHECKLIST_FULL.xlsx"


def main() -> None:
    wb = load_workbook(str(XLSX))
    print(f"Sheets: {len(wb.sheetnames)}")

    total = 0
    pass_count = 0
    fail_count = 0
    skip_count = 0
    blank = 0

    failed_details: list[tuple[str, str, str, str]] = []

    for name in wb.sheetnames:
        if name in ("00_Cover", "01_Summary"):
            continue
        ws = wb[name]
        for row in range(2, ws.max_row + 1):
            cid = ws.cell(row=row, column=1).value
            if not cid:
                continue
            total += 1
            res = ws.cell(row=row, column=9).value
            interaction = ws.cell(row=row, column=3).value or ""
            note = ws.cell(row=row, column=11).value or ""
            if res == "PASS":
                pass_count += 1
            elif res == "FAIL":
                fail_count += 1
                failed_details.append((name, str(cid), str(interaction)[:60], str(note)[:120]))
            elif res == "SKIP":
                skip_count += 1
            else:
                blank += 1

    print(f"Total cases: {total}")
    print(f"  PASS  : {pass_count}")
    print(f"  FAIL  : {fail_count}")
    print(f"  SKIP  : {skip_count}")
    print(f"  blank : {blank}")
    print()
    print("FAIL details:")
    for sheet, cid, inter, note in failed_details:
        print(f"  [{sheet}] {cid} | {inter}")
        print(f"      note: {note}")


if __name__ == "__main__":
    main()
