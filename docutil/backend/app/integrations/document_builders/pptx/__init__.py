"""PPTX 빌더 서브패키지 (Phase 4 S2 D1).

`PptxBuilder` 구체 빌더를 임포트 시점에 `BuilderRegistry` 에 자동 등록한다.
...
테스트에서 Registry 상태를 리셋(clear) 한 경우 다시 등록하려면 아래를 사용::
    ...
    from app.integrations.document_builders.pptx import register_pptx_builder
    register_pptx_builder()

참조:
- docs/phase1_architecture.md §3, §5.3 (PPTX 빌더 설계)
- docs/phase3_execution_roadmap.md §2.2 S2 D1
- backend/app/integrations/document_builders/html/__init__.py (동일 패턴)
"""
...
__all__ = ["PptxBuilder", "register_pptx_builder"]


def register_pptx_builder() -> PptxBuilder:
    """`PptxBuilder` 인스턴스를 생성·등록하고 반환한다.
