...
> **마지막 갱신**: 2026-05-06 (Phase 6.3 코드 작업 완료 — AgentHub Vue 운영자 KB 콘솔 신설. 신규 파일 7 (1 백엔드 컨트롤러 + 1 axios service + 1 Pinia store + 3 Vue page) + 수정 3 (router + i18n ko/en). 백엔드 `dotnet build` 0 errors / 17 pre-existing warnings. 프론트 `vite build` ✓ 3.59s / 신규 청크 9.05 kB. R2 단일 진입점 외부 표면 확보. 직전 Phase 6.4 코드 작업 완료 상태 유지: 자체 KB Models/Services/Controller 7건 `[Obsolete]` 마킹 + RagService 폴백 흐름 한국어 주석 보강. 신규 파일 0 / 수정 7 (`Models/KnowledgeBaseDocument.cs`, `Models/DocumentChunk.cs`, `Services/IKnowledgeBaseService.cs`, `Services/KnowledgeBaseService.cs`, `Services/IDocumentIndexingService.cs`, `Services/DocumentIndexingService.cs`, `Controllers/KnowledgeBaseController.cs`) + 수정 1 (`Services/RagService.cs` 한국어 주석 추가). `dotnet build --no-incremental` 0 errors / **CS0618 신규 11건** (의도적 deprecation, error: false) + 11 pre-existing CS1998. ADR-2 RAG 단일 권위 = DocUtil 의 빌드-시 강제. 외부 시그니처/본문/라우트 보존 — 호출처 회귀 0건. 직전 Phase 6.1+6.2 상태 유지. 마지막 commit `77e7486`)
...
| **다음 Phase** | Phase 6.5 (e2e 검증 — 시연용 DocUtil 운영자 JWT 발급 + `appsettings.Development.json:DocUtil:JwtToken` 주입 + 시드 Agent 1건 KnowledgeBaseSource="DocUtil" SQL UPDATE + `/v1/chat/completions` 호출 시 RAG 결과가 DocUtil 청크에서 오는지 로그 검증 + AgentHub 운영자 콘솔 업로드 → DocUtil 인덱싱 round-trip) / 사이드바 메뉴 통합 (MainLayout 운영자 메뉴 그룹화 — `/admin/*` 카테고리) / vue-tsc 2.x 업그레이드 (Node 24 호환) — 사용자 결정 대기. Phase 8+ 별도 트랙: 자체 KB DB drop + 운영 데이터 DocUtil 마이그레이션 (R20 visibility 매핑 정책 결정 후) |
| **마지막 commit** | `77e7486` (Phase 6.1+6.2 commit 완료 — `[agenthub/docutil-bff] Phase 6.1+6.2 — DocUtilClient + RAG 라우팅 분기 (ADR-2)`). Phase 6.4 commit 대기 — 사용자 승인 후 진행. Phase 4.1~4.5/7.1~7.5 도 commit 누적 — secret leak sanitize 와 함께 별도 트랙) |
...
| **6** | DocUtil 운영자 → AgentHub 흡수 + KB 마이그레이션 | 🔄 진행 중 (6.1+6.2+6.3+6.4 완료 — DocUtil BFF 클라이언트 + RagService 분기 + Vue 운영자 콘솔 + 자체 KB [Obsolete] 마킹 7건. 6.5 e2e 대기) | 2026-05-06 |
