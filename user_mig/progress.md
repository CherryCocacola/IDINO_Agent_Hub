...
| **다음 Phase** | **Phase 6.5 옵션 ③ (DocUtil round-trip)** — Docker 미실행으로 사용자 협조 대기. 절차: Docker Desktop 시작 → `cd docutil && docker compose up -d` → DocUtil 운영자 JWT 발급 → `appsettings.Development.json:DocUtil:JwtToken` 주입(섹션은 신설 완료, 토큰 빈 값) → 시드 Agent 1건 KnowledgeBaseSource="DocUtil" SQL UPDATE → `/v1/chat/completions` 호출 시 RAG 결과가 DocUtil 청크에서 오는지 로그 검증 + AgentHub 운영자 콘솔(`/admin/knowledge-base`) 업로드 → DocUtil 인덱싱 round-trip. 옵션 ④ 시연 종료(현 시점 commit 보존 + Phase 8 보류). 별도 트랙: vue-tsc 2.x 업그레이드 (Node 24 호환) / Phase 8+ 자체 KB DB drop + 운영 데이터 DocUtil 마이그레이션 (R20 visibility 매핑 정책 결정 후) / secret leak history sanitize + force-push (시연용이라 보류) / SQL Server stale 비밀번호 회전 (Phase 3.1 잔재로 화면 노출됨) / appsettings.Development.json LLM API 키 회전 권장 (gitignore 대상이지만 본 세션 grep으로 노출됨) |
| **마지막 commit** | `c9d61a4` (Phase 6.5 — `[agenthub/admin-kb-menu] Phase 6.5 — 사이드바 운영자 KB 메뉴 + vite proxy 보정 + .gitignore 보강`). 이전 commit `d9784f0` (Hangfire fix) / `9801b06` (Phase 6.3) / `055de5a` (Phase 6.4) / `77e7486` (Phase 6.1+6.2) — 모두 누적. Phase 4.1~4.5/7.1~7.5 도 commit 누적 — secret leak sanitize 와 함께 별도 트랙 |
